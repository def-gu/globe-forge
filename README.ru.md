...
- Карточка локации открывает закреплённый там журнал или создаёт новый в один клик.
- Журнал можно перетащить на глобус и привязать к месту. Карточка места его открывает.
- Перетащите актёра на глобус, чтобы разместить токен. Токены сохраняют наземный размер на любом зуме.
- Двигайте свои токены перетаскиванием прямо по глобусу.
- Клик выделяет токен, Alt-клик берёт его в цель, двойной клик открывает лист персонажа.
